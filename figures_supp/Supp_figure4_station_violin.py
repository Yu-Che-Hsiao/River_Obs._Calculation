#!/usr/bin/env python3
# ============================================================
# Supp_figure4_station_violin.py
#   Supplementary version: Fig. 4a station-scale violin plot, record-length threshold robustness check
#
#   Usage
#     python Supp_figure4_station_violin.py --min_years 8
#     python Supp_figure4_station_violin.py --min_years 10
#
#   Dependencies (--min_years 8 as an example)
#     ss_era5_t2m_full/obs_trends_8y.csv   ss_era5_skt_full/obs_trends_8y.csv
#     ss_hadisd_full/obs_trends_8y.csv     ss_dynwat_full/obs_trends_8y.csv
#     ss_gemstat_trends_8y.csv
#   Produced by calc_station_scale.py          --min_years 8 and
#               calc_make_ss_dynwat_full_v2.py --min_years 8 
#
#   Output (figures/Supplementary/)
#     Supplementary_Fig4a_station_violin_{ny}y.png
#     Supplementary_station_scale_stats_{ny}y.csv
#
#   Original v1.2/v1.3/v1.4 notes follow
# ------------------------------------------------------------
# Main_Text_Figure4_station_violin_v1.2.py   —  station scale (violin + box) standalone file (formerly Fig3)
#   v1.3: (1) sig changed from "any month p<0.05" to "significant months within the season share the same sign", split into sig_warm / sig_cool
#         (2) added station-level DynWat TR ALL (reads ss_dynwat_full_trends_5y.csv; falls back to the global grid if the file is missing)
#         (3) outputs station_scale_stats.csv: n, median, IQR, and significant warming/cooling fractions for each group and season
#   v1.2: section label keeps only 'a' (removed the "station scale" text).
...
#   #8 red/blue kept at the current best pairing, fixed as the figure-wide shared base (RED_BASE / BLUE_BASE)
...
_ap.add_argument('--min_years', type=int, default=8,
                 help='record-length threshold, 8 or 10; for 5 use the main text v1.4')
...
# ── Dependency pre-check (if a file is missing, state exactly which script to run)──────────────
_need = [f'ss_era5_t2m_full_trends_{MIN_YEARS}y.csv',
         f'ss_era5_t2m_obs_trends_{MIN_YEARS}y.csv',
         f'ss_hadisd_full_trends_{MIN_YEARS}y.csv',
         f'ss_hadisd_obs_trends_{MIN_YEARS}y.csv',
         f'ss_era5_skt_full_trends_{MIN_YEARS}y.csv',
         f'ss_era5_skt_obs_trends_{MIN_YEARS}y.csv',
         f'ss_dynwat_obs_trends_{MIN_YEARS}y.csv',
         f'ss_gemstat_trends_{MIN_YEARS}y.csv']
_miss = [f for f in _need if not os.path.exists(DATA_DIR + f)]
if _miss:
    raise FileNotFoundError(
        'The following station-scale trend files are missing:\n  ' + '\n  '.join(_miss) +
        f'\n\nRun this first:\n  python calc_station_scale.py --min_years {MIN_YEARS}')
print(f'MIN_YEARS = {MIN_YEARS}', flush=True)

# ── Figure-wide shared base colors (violin red/blue; #8)──────────────────────
...
# ════════════════════════════ Data ════════════════════════════
print('Preparing violin data...', flush=True)
def ss_to_seasonal(path, group):
    """Seasonal significance: a station is judged significantly warming or significantly cooling only when all significant months in that season share the same sign.
    The old version (v1.2) used `(p<0.05).any()`, running 3 tests over 3 months without regard to sign;
    under the null the family-wise error rate is about 1-0.95**3 = 14%, counting noise as significant."""
    ...
    for sid, grp in df.groupby('station_id'):
        ...
        for sea, mdef in SEASONS.items():
            ...
            if has_p:
                ...
                sig_any  = bool(sig_m.any())          # old definition, kept only for comparison
            else:
    ...
    print(f'  {group}: {r["station_id"].nunique()} stations', flush=True); return r
...
# DynWat TR ALL: prefer the complete record at the 219 station locations, so it can be compared under controlled conditions with the other columns in the figure.
# Falls back to the global grid when the file is missing (= v1.2 behavior), and prints a warning.
...
if os.path.exists(DW_FULL):
    ...
    print(f'  [Warning] {DW_FULL}not found; DynWat TR ALL falls back to the global grid,'
          f'a different spatial population from the other columns, not usable for a controlled comparison.', flush=True)
...
def draw_violin(ax, txt_c):
    ...
    # #7 removed the central dashed divider (formerly the sep axvline) —— intentionally not drawn here
    ...
    # label colors aligned with the violins
    SUMMER_COLOR = '#D6604D'   # boreal summer red
    WINTER_COLOR = '#4393C3'   # boreal winter blue
    # label y positions (data coordinates); axis bottom -5, labels placed below it with extra line spacing
    LBL_Y_SUM = -5.9
    LBL_Y_WIN = -6.75
    for g in GROUP_ORDER:
        xc = GXPOS[g]
        ...
        if has_sig:
            ...
            lbl_sum = f'{n_sum}'
            lbl_win = f'{n_win}'
        # x aligned with the violins; summer (top row, red), winter (bottom row, blue). y in data coordinates, line spacing 0.75
    ...
    # #7 removed the "← Air / Water →" text —— intentionally not drawn here
    
# ════════════════════════════ Layout ════════════════════════════
def make_fig():
    bg='white'; txt='#111111'
    fig=plt.figure(figsize=(10,4.5),facecolor=bg)
    # single panel; left/right margins aligned with the other two figures; bottom whitespace for x labels, station counts, legend
...
stats = dump_stats(FIG_DIR + f'Supplementary_station_scale_stats_{MIN_YEARS}y.csv')
print(stats[['group','season','n','median','iqr','pct_sig_warm','pct_sig_cool','pct_sig_any_oldrule']].to_string(index=False))

print('Plot (station violin)...', flush=True)
fw=make_fig()
out=FIG_DIR+f'Supplementary_Fig4a_station_violin_{MIN_YEARS}y.png'
