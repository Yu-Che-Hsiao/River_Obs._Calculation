#!/usr/bin/env python3
# ============================================================
# Supp_FigS4_Main_Text_Figure2_time_series_mean_v1.0.py
#   Supplementary version: record-length threshold robustness check (8y / 10y)
#
#   Usage (one script runs both thresholds)）
#     python Supp_FigS4_Main_Text_Figure2_time_series_mean_v1.0.py --min_years 8
#     python Supp_FigS4_Main_Text_Figure2_time_series_mean_v1.0.py --min_years 10
#
#   ⚠ Important note
#   In the original v1.4, MIN_YEARS was used only in the output filename and took no part in any data filtering.
#   The aggregated-scale anomaly series (regional_series_v1.csv) is itself independent of the threshold,
#   and the right-axis trend points were hard-coded to valid.sum() >= 5. This version wires that to MIN_YEARS,
#   but the global/continental series span 31 years (1990-2020), so >=8 and >=10 still all pass,
#   so this figure is very likely identical under all three thresholds. Compare md5 after running before deciding whether to include it in the SI.
#
#   The original v1.3/v1.4 notes follow
# ------------------------------------------------------------
...
_ap.add_argument('--min_years', type=int, default=5,
                 help='record-length threshold; the mean version uses 5 for comparison')
_ap.add_argument('--agg', choices=['median','mean'], default='mean',
                 help='cross-station aggregation method; this file defaults to mean, versus median in the main text')
...
# calc_scale_trends_v1.py 加上 FNAME_SUFFIX 之後，5y 維持原檔名、其餘帶年份後綴
SUFFIX = ('' if MIN_YEARS == 5 else f'_{MIN_YEARS}y') + ('' if AGG=='median' else '_mean')
...
_ts_pref = SCALE_DIR + f'regional_series_v1{SUFFIX}.csv'
_ts_base = SCALE_DIR + 'regional_series_v1.csv'
if os.path.exists(_ts_pref):
    TS_PATH = _ts_pref
else:
    TS_PATH = _ts_base
    print(f'  ⚠ {os.path.basename(_ts_pref)}not found; falling back to  {os.path.basename(_ts_base)}。\n'
          f'    The aggregated anomaly series is threshold-independent anyway, but please confirm this is the intended behavior.', flush=True)
print(f'Read {os.path.basename(TS_PATH)}(per-station anomaly, per-station hemisphere)...', flush=True)
...
fw = make_fig()
out = FIG_DIR + f'Supplementary_Fig2_time_series_{MIN_YEARS}y_{AGG}.png'
