#!/usr/bin/env python3
# ============================================================
# Supp_FigS9_Main_Text_Figure3_heatmap_mean_v1.0.py
#   Supplementary version: record-length threshold robustness check (8y / 10y)）
#
#   Usage (one script runs both thresholds)
#     python Supp_FigS9_Main_Text_Figure3_heatmap_mean_v1.0.py --min_years 8
#     python Supp_FigS9_Main_Text_Figure3_heatmap_mean_v1.0.py --min_years 10
#
#   Prerequisites
...
_ap.add_argument('--min_years', type=int, default=5,
                 help='record-length threshold; use 5 for the mean-version comparison')
_ap.add_argument('--agg', choices=['median','mean'], default='mean',
                 help='cross-station aggregation method; this file defaults to mean, comparing against the main text median')
...
N_LEVELS = 6      # 6 segments (0.5 each): every tick is exactly one transition line. For a finer gradient use 12 (0.25 each, ticks land on every other transition line)
...
print('reparing heatmap data...', flush=True)
...
print(f'MIN_YEARS = {MIN_YEARS}(reading suffix "{SUFFIX or "no"}")', flush=True)
_missing=[f for f,_ in HROWS_FILES.values() if not os.path.exists(SCALE_DIR+f)]
if _missing:
    raise FileNotFoundError(
        'he following basin trend files are missing:\n  ' + '\n  '.join(_missing) +
        f'\n\nApply the FNAME_SUFFIX change first, then run:\n'
        f'  python calc_scale_trends_mean.py --min_years {MIN_YEARS} --agg {AGG} --from_cache')
...
#    ax.set_title(title,color=txt_c,fontsize=9,pad=2,loc='left',fontweight='normal')
    if show_sw_label:                                                                        # Build a blended coordinate system: X uses the figure's absolute fraction (0~1), 
        ...
        x_pos = 0.985                                                                        # plus the previous figure's Trend Y-axis ticks and labelpad, it lands roughly between 0.98 ~ 0.99.
...
print('Plot (basin heatmap)...', flush=True)
fw=make_fig()
out=FIG_DIR+f'Supplementary_Fig3_heatmap_{MIN_YEARS}y_{AGG}.png'
