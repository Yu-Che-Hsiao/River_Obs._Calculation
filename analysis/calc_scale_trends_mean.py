...
# Design:
#   scale：
#     - basin       : Main Basin from metadata
...
FNAME_SUFFIX = ''   # set by main() from --min_years; empty for 5y, _Ny otherwise
AGG = 'median'      # set by main() from --agg; aggregation method for cross-station / regional / global
...
def wagg_by_mode(values, weights):
    """Choose weighted median or weighted mean according to the global AGG setting."""
    return weighted_mean(values, weights) if AGG == 'mean' else weighted_median(values, weights)
...
# ============================================================
# 0. 0. Station list + grouping (basin / continental)
# ============================================================
def load_stations(station_set):
    print('Rㄎ station metadata...', flush=True)
    ...
    if station_set == 'qc219':
        ...
        print(f'  All River station:{len(meta)} stations', flush=True)
...
def extract_era5_t2m(si, station_dates):
    ...
    if nc4 is None:
        raise RuntimeError('netCDF4 kit needed')
...
def scale_trends(long_df, si, min_years):
    ...
    for scale in SCALES:
        ...
        def _wagg_mean(sub):
            import pandas as pd
            return pd.Series({
                'anom':   wagg_by_mode(sub['anom'].values, sub['w'].values),
                'absmed': wagg_by_mode(sub['val'].values,  sub['w'].values),
            })
        agg = (gg.groupby([scale,'year','month']).apply(_wagg_mean).reset_index())
    ...
    for season, hd in SEAS_MONTHS_TS.items():
        ...
        for region in CONTINENTS_TS:
            ...
            for yr, sub in rr.groupby('year'):
                v = wagg_by_mode(sub['anom'].values, sub['w'].values)
...
def write_out(trends, dataset, version):
    ...
    for scale, df in trends.items():
        ...
        print(f'  → {tag}.csv  ({len(df)} 筆, {n} 組, median={med:.3f})', flush=True)
...
def main():
    ...
    ap.add_argument('--agg', choices=['median', 'mean'], default='median',
                    help='Aggregation method for cross-station / regional / global; within-station monthly values and the anomaly baseline are always median')
    ...
    AGG = args.agg
    global FNAME_SUFFIX
    FNAME_SUFFIX = ('' if MY == 5 else f'_{MY}y') + ('' if AGG == 'median' else '_mean')
    FC = args.from_cache
    ...
    if reg_parts:
        ...
        base.to_csv(OUT_DIR + 'regional_series_v1' + FNAME_SUFFIX + '.csv', index=False)
        scols = [c for c in base.columns if c not in ('region', 'season', 'year')]
        print(f'\n  → regional_series_v1{FNAME_SUFFIX}.csv  ({len(base)} records, columns={scols})', flush=True)
