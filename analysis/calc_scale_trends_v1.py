...
#
# Recompute monthly trends consistently at three scales: basin / continental (5 continents) / global.
# All datasets follow "extract at stations → cross-station median → per-calendar-month OLS", aligned with GEMStat.
#
# Design:
#   scale:
#     - basin       ：metadata from Main Basin
#     - continental ：5 continents (via the lat/lon boxes in assign_region)
#     - global      ：all stations as one group
#   version:
#     - GEMStat (water): single version (it is itself the observation)
#     - ERA5 TA / ERA5 TS / HadISD / DynWat: each has ALL + SUB
#   SUB is aligned to GEMStat, each dataset matched at the finest resolution its data allows:
#     - ERA5 t2m  (hourly): matched per DAY — take daily means only on the days GEMStat actually sampled that station-month
#     - HadISD (sub-daily): matched per DAY
#     - ERA5 SKT (monthly): matched per MONTH — keep only (station, year, month) that GEMStat observed
#     - DynWat   (monthly): matched per MONTH
#   aggregation is always the median:
#     - within-station monthly value: GEMStat = median of that month's samples; day-matched sources = median of the daily means on sampled days;
#                     monthly sources = that month's value
#     - cross-station: median across stations for each (group, year, month)
#   trend: OLS over years for each (group, calendar month) (>=MIN_YEARS years), output in °C/decade
#
# Output (new subfolder, does not overwrite old files):
...
#     version ∈ {all, sub}（gemstat has no version）
#
# Usage:
#   python calc_scale_trends_v1.py                  # all datasets
#   python calc_scale_trends_v1.py --dataset era5_skt dynwat gemstat   # run a subset only
#   python calc_scale_trends_v1.py --min_years 8
#   python calc_scale_trends_v1.py --station_set qc219   # use the v5y 219-station set
#
# Note: ERA5 t2m hourly files are heavy and slow; run the fast ones first via --dataset (gemstat/dynwat/era5_skt).
# ============================================================
...
# ── Added: place near the top of the file (after imports, before functions)──────────────
def weighted_median(values, weights):
    """cos-latitude weighted median: sort, then find the value where cumulative weight reaches 50% of the total."""
    import numpy as np
...
# ── 5 continents (consistent with assign_region in the Fig2 combine script) ──────────────
def assign_region(lat, lon):
    ...
    return None   # Other →  not included in continental.
...
def detect_var(ds, keywords):
    ...
    if len(cands) == 1:
        return cands[0]
    raise KeyError(f'{keywords}not found; available: {list(ds.data_vars)}')
...
def detect_time_dim(ds):
    for c in ('time', 'valid_time', 'date', 'dates'):
        if c in ds.coords or c in ds.dims:
            return c
    raise KeyError(f'Time dimension not found; coords={list(ds.coords)}')


# ============================================================
# 0. Station list + grouping (basin / continental)
# ============================================================
def load_stations(station_set):
    ...
    if station_set == 'qc219':
        ...
        print(f'  Applying v5y 219-station list:{len(meta)} stations', flush=True)
    else:
        print(f'  All river stations: {len(meta)} stations', flush=True)
...
# ============================================================
# 1. GEMStat observation index (for SUB matching) + GEMStat monthly values
# ============================================================
def build_gemstat(si):
    print('[GEMStat] Reading raw data, building observation index...', flush=True)
    ...
    # Set of sampled days per station-month (for day-matched SUB)
    ...
    station_months = set(station_dates.keys())      # for month-matched SUB
    print(f'  (station, year, month) combinations:{len(station_months):,}', flush=True)

    # GEMStat monthly value = median of that month's samples
...
# ============================================================
# 2. Per-dataset extraction (returns long: station_id, year, month, val)
# ============================================================
def extract_era5_t2m(si, station_dates):
    print('[ERA5 t2m] Extracting from hourly files (ALL monthly mean + SUB day-matched)...', flush=True)
    if nc4 is None:
        raise RuntimeError('netCDF4 kit required')
    ...
    for k, f in enumerate(files):
        ...
        # First compute daily means (hourly → daily)
        ...
        # ALL: median of all daily means in the month (median, consistent with SUB and GEMStat)
        monthly = np.nanmedian(daily, axis=0)
        for s, sid in enumerate(sids):
            if np.isfinite(monthly[s]):
                all_rows.append((sid, yr, mo, float(monthly[s])))
        # SUB: median over only the days sampled at that station-month
        for s, sid in enumerate(sids):
...
# ============================================================
# ── Replaces scale_trends (entire function)────────────────────────────
def scale_trends(long_df, si, min_years):
    """Returns {scale: DataFrame}. long_df: station_id, year, month, val.
       Cross-station aggregation uses cos-latitude weighted median (both anomalies and absolute values are weighted)."""
    ...
    g['global'] = 'GLOBAL'
    # Added: bring in each station's latitude → cos weight (the latitude column in si is 'lat'
    g['lat'] = g['station_id'].map(gmap['lat'])
    g['w'] = np.cos(np.radians(g['lat']))
 
    clim = g.groupby(['station_id', 'month'])['val'].transform('median')  # climatology baseline: unchanged
    g['anom'] = g['val'] - clim
    ...
    for scale in SCALES:
        ...
        if gg.empty:
            ...
            continue
        # Cross-station cos-weighted median: weighted median of anom and val for each (group, year, month)
        def _wagg(sub):
            return pd.Series({
                'anom':   weighted_median(sub['anom'].values, sub['w'].values),
                'absmed': weighted_median(sub['val'].values,  sub['w'].values),
            })
        agg = (gg.groupby([scale, 'year', 'month'])
                 .apply(_wagg)
                 .reset_index())
...
# ── Replaces regional_series (entire function) ─────────────────────────
def regional_series(long_df, si):
    """Cross-station cos-weighted median anomaly for each (region, season, year). region = 5 continents + Global."""
    import numpy as np, pandas as pd
    ...
    g['w'] = np.cos(np.radians(g['lat']))   # cos weight
    ...
    for season, hd in SEAS_MONTHS_TS.items():
        ...
        # Per-station seasonal anomaly = mean of that season's monthly anomalies (within-station, unchanged; weight carried along
        sta = (gs.groupby(['station_id', 'continental', 'year'])
                 .agg(anom=('anom', 'mean'), w=('w', 'first'))
                 .reset_index())
        ...
        for region in CONTINENTS_TS:
            ...
            for yr, sub in rr.groupby('year'):
                val = weighted_median(sub['anom'].values, sub['w'].values)  # cross-station weighted median
                out.append({'region': region, 'season': season, 'year': int(yr), 'value': float(val)})
        for yr, sub in sta.groupby('year'):   # Global = all stations, cross-station weighted median
            val = weighted_median(sub['anom'].values, sub['w'].values)
            out.append({'region': 'Global', 'season': season, 'year': int(yr), 'value': float(val)})
...
def write_out(trends, dataset, version):
    ...
    for scale, df in trends.items():
        tag = f'{dataset}_{scale}' + (f'_{version}' if version else '')
        ...
        print(f'  → {tag}.csv  ({len(df)} records, {n} groups, median={med:.3f})', flush=True)
...
def cached_extract(name, extractor, eargs, from_cache):
    ...
    if from_cache and os.path.exists(pa) and os.path.exists(ps):
        print(f'  [cache] Reading {name} station-year-month intermediate table (skip re-extraction)', flush=True)
...
def main():
    ...
    print(f'\nAll done! Output in {OUT_DIR}', flush=True)
