#!/usr/bin/env python3
# ============================================================
# Main_Text_Figure3_heatmap_v1.2.py - standalone basin-scale figure (formerly Fig 2g, renamed Fig 3 after splitting)
# v1.2 changes: removed "g" from the title, keeping only "basin scale"; colorbar moved down (CB_YSHIFT) to avoid overlapping the bottom-row basin labels;
#           colorbar narrowed (CB_WFRAC) and thinned (CB_HFRAC).
# Split out from FIG2_combine_figure_v2.5.py, with v2.7 changes applied:
#   #5 outer frame thinned (lw 1.5), the summer/winter divider thinned to match; rightmost Summer/Winter no longer bold
#   #6 colorbar switched to nrl_sirkes (or rdbu) from the colormaps package
...
MIN_YEARS = 5
SCALE_DIR = '/work/home/H.Jason421/water_temp_for_publish/data/scale_trends_v1/'   # newly computed basin/continental/global trends are all here
...
def build_hcmap(style):
    ...
    if style == 'violin':                                           # ends use the violin blue/red
...
def drop_white(cmap, gap=0.16, n=256):
    # Cut out the white band in the palette center and reassemble; larger gap = less white in the middle, sharper blue/red separation (gap=0 keeps the white)
...
# ── colorbar adjustable layout constants (v1.2: moved down and narrowed to avoid overlapping the bottom-row basin labels) ──
CB_WFRAC  = 0.50   # colorbar length as a fraction of the row width (smaller = narrower; was 2/3)
CB_YSHIFT = 0.0    # padding is handled mainly by the spacer row; no extra shift here
CB_HFRAC  = 0.80   # colorbar thickness as a fraction of the gridspec row height (smaller = thinner)

HVMAX = 1.5
# v1.1: cells and colorbar share a discrete color scale, with boundaries at evenly spaced points over -1.5..1.5,
# so the ticks at -1.5/-1.0/-0.5/0/0.5/1.0/1.5 fall exactly on the color boundaries (and boundaries appear only at ticks).
N_LEVELS = 6      # 6 levels (0.5 each): every tick is exactly one boundary. For a finer gradient use 12 (0.25 each; ticks fall on every other boundary)
HBOUNDS    = np.linspace(-HVMAX, HVMAX, N_LEVELS + 1)
HCMAP_FULL = drop_white(build_hcmap(CB_STYLE), gap=0.16)                     # continuous parent colormap
HCMAP      = mcolors.ListedColormap(HCMAP_FULL(np.linspace(0, 1, N_LEVELS))) # discrete: shared by cells and colorbar
HCMAP.set_over(HCMAP_FULL(1.0)); HCMAP.set_under(HCMAP_FULL(0.0))
HNORM      = mcolors.BoundaryNorm(HBOUNDS, N_LEVELS, clip=True)              # colors the cells (out-of-range values clipped to the ends)
...
HROWS_FILES = {
    'ERA5 TA ALL':   ('era5_t2m_basin_all.csv','trend_per_decade'),
    'ERA5 TA SUB':   ('era5_t2m_basin_sub.csv','trend_per_decade'),
    'HadISD TA ALL': ('hadisd_basin_all.csv','trend_per_decade'),
    'HadISD TA SUB': ('hadisd_basin_sub.csv','trend_per_decade'),
    'ERA5 TS ALL':   ('era5_skt_basin_all.csv','trend_per_decade'),
    'ERA5 TS SUB':   ('era5_skt_basin_sub.csv','trend_per_decade'),
    'DynWat TR ALL': ('dynwat_basin_all.csv','trend_per_decade'),
    'DynWat TR SUB': ('dynwat_basin_sub.csv','trend_per_decade'),
    'GEMStat TR':    ('gemstat_basin.csv','trend_per_decade'),
}
...
def draw_heatmap_region(ax,HM_s,SIG_s,HM_w,SIG_w,col_indices,title,txt_c,bg_c,
    ...
    # #5 summer/winter divider 2.5→1.5
...
#    ax.set_title(title,color=txt_c,fontsize=9,pad=2,loc='left',fontweight='normal')
    if show_sw_label:                                                                                 # Build a blended coordinate system: X uses figure fraction (0~1), Y uses 
                                                                                                      # the heatmap's data coordinates
        import matplotlib.transforms as mtransforms
        trans = mtransforms.blended_transform_factory(ax.figure.transFigure, ax.transData)            # X set to 0.985. Since the figure's right margin is 0.96,
        x_pos = 0.985                                                                                 # plus the previous figure's Trend y-axis ticks and labelpad, it lands
                                                                                                      # roughly between 0.98 and 0.99.
...
def make_fig():
    ...
    # 5 rows: hm_top, hm_gap, hm_bot, label_spacer (for the bottom-row basin labels), colorbar
    # left/right margins aligned with the other two figures; the spacer keeps the colorbar off the basin labels
    ...
    # colorbar: length shrunk to 2/3 of the row width, centered; shares the discrete color scale with the cells → boundaries only at ticks
...
fw=make_fig()
out=FIG_DIR+f'Main_Text_Figure3_heatmap_v1.2_{MIN_YEARS}y_white.png'
